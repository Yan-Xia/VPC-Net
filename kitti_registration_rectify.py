...
def register(source, target, args):
    residual = open3d.registration.TransformationEstimationPointToPoint()
    criteria = open3d.registration.ICPConvergenceCriteria(max_iteration=args.max_iter)
    ...
    source.points = open3d.utility.Vector3dVector(source_points - source_center)
    target = open3d.geometry.PointCloud()
    target.points = open3d.utility.Vector3dVector(target_points - target_center)
    result = open3d.registration.registration_icp(source, target, args.max_dist, np.eye(4), residual, criteria)
...
def track(args):
    ...
    total_r_err_comp = 0
    total_t_err_comp = 0
    #args.tracklet_dir = sort(args.tracklet_dir)
    for filename in sorted(os.listdir(args.tracklet_dir)):
        tracklet_id = filename.split('.')[0]
        if int(tracklet_id.split('_')[1]) == 0:
            continue
        ...
        for i in range(len(car_ids)):
            if i < len(car_ids) -1 :
                if car_ids[i].split('_')[3] == car_ids[i+1].split('_')[3]:
                    group_index[-1].append(car_ids[i])
                else:
                    group_index[-1].append(car_ids[i])
                    group_index.append([])
        group_index[-1].append(car_ids[i])
        ...
        for i in range(len(group_index)):
            prev_frame = int(group_index[i][0].split('_')[1])
            print ('prev_frame', prev_frame)
            prev_R, prev_t = bbox2rt(np.loadtxt(os.path.join(args.bbox_dir, '%s.txt' % group_index[i][0])))
            prev_partial = open3d.io.read_point_cloud(os.path.join(args.partial_dir, '%s.pcd' % group_index[i][0]))
            print ('prev_partial', os.path.join(args.partial_dir, '%s.pcd' % group_index[i][0]))      
            prev_complete = open3d.io.read_point_cloud(os.path.join(args.complete_dir, '%s.pcd' % car_ids[0]))
            for p in range(args.interval, len(group_index[i]), args.interval):
                n += 1
                frame = int(group_index[i][p].split('_')[1])
                print (frame)
                instance_id = '%s_frame_%d_to_%d' % (tracklet_id, prev_frame, frame)
                print (instance_id)
                print (group_index[i][p])
                
                R, t = bbox2rt(np.loadtxt(os.path.join(args.bbox_dir, '%s.txt' % group_index[i][p])))
                print (os.path.join(args.bbox_dir, '%s.txt' % group_index[i][p]))
                #exit()
                R_gt = np.dot(R, prev_R.T)
                t_gt = t - np.dot(prev_t, R_gt.T)

                partial = open3d.io.read_point_cloud(os.path.join(args.partial_dir, '%s.pcd' % group_index[i][p]))
                print ('partial', os.path.join(args.partial_dir, '%s.pcd' % group_index[i][p]))
                #exit()
                ...
                complete = open3d.io.read_point_cloud(os.path.join(args.complete_dir, '%s.pcd' % group_index[i][p]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--complete_dir', default='./results/kitti_pcn_STN_PFL_refinenet/completions')
