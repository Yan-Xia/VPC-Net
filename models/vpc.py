...
class Model:
    def __init__(self, inputs, npts, gt, alpha, is_training):
        ...
        self.coarse, self.fine, self.fine_new = self.create_decoder(self.features, inputs, npts,is_training)
    ...
    def create_decoder(self, features, inputs, npts, is_training):
        ...
        with tf.variable_scope('folding', reuse=tf.AUTO_REUSE):
            ...
            origin = tf.split(inputs, npts, axis=1)
            fine_concat = [tf.concat([fine[i], tf.squeeze(origin[i], axis = 0)], axis = 0)
                    for i,f in enumerate(origin)]
            fine_concat = [gather_point(tf.expand_dims(i, axis = 0), farthest_point_sample(self.num_fine, tf.expand_dims(i, axis = 0)))
                        for i in fine_concat]
            fine_concat = tf.concat(fine_concat, axis = 0)

            fine_features_1 = mlp_conv_single_bn(fine_concat, [64], is_training)
            ...
            fine_new = fine_concat + fine_features_9
