...
def track(args):
    ...
    for filename in os.listdir(args.tracklet_dir):
        ...
        prev_partial = open3d.io.read_point_cloud(os.path.join(args.partial_dir, '%s.pcd' % car_ids[0]))
        ...
        for i in range(args.interval, len(car_ids), args.interval):
            ...
            partial = open3d.io.read_point_cloud(os.path.join(args.partial_dir, '%s.pcd' % car_ids[i]))
            ...
            complete = open3d.io.read_point_cloud(os.path.join(args.complete_dir, '%s.pcd' % car_ids[i]))
